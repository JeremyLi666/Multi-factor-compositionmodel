...
def run_task(dataset_id, region, delay, instrumentType, universe, n_jobs, tag=None):
    ...
    print(datetime.now(), f"================= Digging Consultant STEP3 ==================")
    ...
    for step2_tag in step2_tag_list:
        ...
        if len(so_layer) == 0:
            print(datetime.now(),'暂时没有满足条件的二阶段因子，请你继续运行digging_consultant_2step.py.')
            continue

        print(datetime.now(),f'qualified expression: {len(so_layer)}')
